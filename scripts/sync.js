...
const transUrl = `https://${username}:${token}@github.com/${transOwner}/${transRepository}.git`;
...
if (shell.cd(transRepository).code !== 0) {
  logger.info("Can't find translation repo locally. Cloning...");
  shell.exec(`git clone ${transUrl} ${transRepository}`);
  logger.debug("Finished cloning.");
  shell.cd(transRepository);
